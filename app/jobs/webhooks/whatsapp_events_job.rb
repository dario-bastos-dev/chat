class Webhooks::WhatsappEventsJob < MutexApplicationJob
  ...
  def get_channel_from_wb_payload(wb_params)
    display_phone_number = wb_params[:entry].first[:changes].first.dig(:value, :metadata, :display_phone_number).to_s
    phone_number = display_phone_number.start_with?('+') ? display_phone_number : "+#{display_phone_number}"
