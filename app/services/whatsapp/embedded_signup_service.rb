class Whatsapp::EmbeddedSignupService
  ...
  def perform
    ...
    # Must run after setup_webhooks: the history and address book payloads are delivered to the phone-level
    # callback, so the override has to be registered before Meta starts pushing them.
    Channels::Whatsapp::CoexistenceSyncJob.perform_later(channel)
