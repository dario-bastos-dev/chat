...
class AutomationRule < ApplicationRecord
  ...
  def conditions_attributes
    %w[content email country_code status message_type browser_language assignee_id team_id referer city company inbox_id
       mail_subject phone_number priority conversation_language labels is_first_message has_active_deal deal_stage_id deal_labels deal_custom_attribute]
  end
  ...
  def actions_attributes
    %w[send_message add_label remove_label send_email_to_team assign_team assign_agent send_webhook_event mute_conversation
       ...
       send_email_transcript add_private_note create_deal update_deal_info move_deal_stage sync_deal_assignee change_deal_status add_deal_label remove_deal_label].freeze
  end
